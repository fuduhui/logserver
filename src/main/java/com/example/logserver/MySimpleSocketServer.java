 ...
 * @since 0.8.4
 */
public class MySimpleSocketServer extends Thread {

    Logger logger = LoggerFactory.getLogger(ch.qos.logback.classic.net.SimpleSocketServer.class);

    private final int port;
    private final LoggerContext lc;
    private boolean closed = false;
    private ServerSocket serverSocket;
    private List<MySocketNode> socketNodeList = new ArrayList<MySocketNode>();

    // used for testing purposes
    private CountDownLatch latch;

    public static void main(String argv[]) throws Exception {
        doMain(ch.qos.logback.classic.net.SimpleSocketServer.class, argv);
    }

    protected static void doMain(Class<? extends ch.qos.logback.classic.net.SimpleSocketServer> serverClass, String argv[]) throws Exception {
        int port = -1;
        if (argv.length == 2) {
       ...
       //        String configFile="E:\\workspace\\demo2\\src\\main\\resources\\server1.xml";

     //   int port=6001;
      //  String configFile=AppLogUtil.getConfig();

        LoggerContext lc = (LoggerContext) LoggerFactory.getILoggerFactory();
        configureLC(lc, configFile);

         MySimpleSocketServer sss = new MySimpleSocketServer(lc, port);
         sss.start();
    }
    ...
    public void run() {
        ...
        try {
            ...
            while (!closed) {
                ...
                MySocketNode newSocketNode = new MySocketNode(this, socket, lc);
    ...
    public void close() {
        ...
        synchronized (socketNodeList) {
            for (MySocketNode sn : socketNodeList) {
                sn.close();
            }
        }
    ...
    public void socketNodeClosing(MySocketNode sn) {
    ...
    static void usage(String msg) {
        ...
        System.err.println("Usage: java " + ch.qos.logback.classic.net.SimpleSocketServer.class.getName() + " port configFile");
        System.exit(1);
    }
